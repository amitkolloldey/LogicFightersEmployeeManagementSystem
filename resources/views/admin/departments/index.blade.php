...
@section('content')
    ...
    <p>
        <a href="{{ route('admin.departments.create') }}" class="btn btn-success">@lang('lfems.lf_add_new')</a>
    ...
    <p>
        <ul class="list-inline">
            <li><a href="{{ route('admin.departments.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('lfems.lf_all')</a></li> |
            <li><a href="{{ route('admin.departments.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('lfems.lf_trash')</a></li>
        </ul>
    </p>
    @endcan
                    ...
                    @if (count($departments) > 0)
                        @foreach ($departments as $department)
                            <tr data-entry-id="{{ $department->id }}">
                                ...
                                <td>
                                    @can('department_view')
                                    <a href="{{ route('admin.departments.show',[$department->id]) }}" class="btn btn-xs btn-primary">@lang('lfems.lf_view')</a>
                                    @endcan
                                    @can('department_edit')
                                    <a href="{{ route('admin.departments.edit',[$department->id]) }}" class="btn btn-xs btn-info">@lang('lfems.lf_edit')</a>
                                    @endcan
                                    @can('department_delete')
                                    {!! Form::open(array( 'style' => 'display: inline-block;','method' => 'DELETE','onsubmit' => "return confirm('".trans("lfems.lf_are_you_sure")."');",'route' => ['admin.departments.destroy', $department->id])) !!}
